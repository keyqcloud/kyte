...
class BaseObject
{
	...
	protected function bindTypes($params) {
		...
		foreach ($params as $key => $value) {
			if (array_key_exists($key, $this->table['struct'])) {
				...
				$types .= $this->table['struct'][$key]['type'];
			}
		}
	...
	protected function validateRequiredParams($params) {
		if (count($params) == 0) {
			...
			foreach ($this->table['struct'] as $key => $value) {
	...
	public function create($params)
	{
		...
		try {
			...
			$id = DBI::insert($this->table['name'], $params, $types);
			$this->populate($id);
	...
	public function save($params)
	{
		...
		try {
			DBI::update($this->table['name'], $id, $params, $types);
			return true;
		} catch (\Exception $e) {
			throw $e;
			return false;
	...
	public function populate($id = null)
	{
		try {
			...
			$data = DBI::select($this->table['name'], $id);
	...
	public function delete($field = null, $value = null)
	{
		try {
			if (isset($field, $value)) {
				$data = DBI::select($this->table['name'], null, "WHERE `$field` = '$value'");
				if (!isset($data[0]['id'])) {
					$id = $data[0]['id'];
				} else {
			...
			DBI::update($this->table['name'], $id, ['deleted' => 1], 'i');
